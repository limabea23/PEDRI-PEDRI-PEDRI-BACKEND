...
const createGaleria = async (req, res) => {
    try {
        const { listagem_id, titulo } = req.body;
        const photo = req.file ? req.file.filename : null;
        const newGaleria = await galeriaModel.createGaleria(listagem_id, titulo, photo);
...
const updateGaleria = async (req, res) => {
    try {
        const { listagem_id, titulo } = req.body;
        const photo = req.file ? req.file.filename : null;
        const updatedGaleria = await galeriaModel.updateGaleria(req.params.id, listagem_id, titulo, photo);
        if (!updatedGaleria) {
            return res.status(404).json({ message: "galeria não encontrada." });
        }
